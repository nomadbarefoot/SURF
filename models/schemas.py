...
class SessionCreateRequest(BaseModel):
    ...
    user_agent: Optional[str] = Field(
        default=None, max_length=500, description="Custom user agent"
    )
    ...
    silent: Optional[bool] = Field(
        default=None,
        description="Run browser in the background without opening a visible window",
    )
...
class NavigateRequest(BaseModel):
    ...
    wait_until: WaitUntil = Field(
        default=WaitUntil.DOMCONTENTLOADED, description="Wait condition"
    )
...
class ExtractRequest(BaseModel):
    ...
    selector: Optional[str] = Field(
        default=None, max_length=1000, description="CSS selector"
    )
    timeout: Optional[int] = Field(
        default=None, ge=1000, le=60000, description="Timeout in milliseconds"
    )
...
class ScreenshotRequest(BaseModel):
    ...
    session_id: str = Field(..., description="Session ID")
    selector: Optional[str] = Field(
        default=None, max_length=1000, description="CSS selector for element screenshot"
    )
    full_page: bool = Field(default=False, description="Capture full page")
    path: Optional[str] = Field(
        default=None, max_length=500, description="Custom file path"
    )
    quality: Optional[int] = Field(
        default=None, ge=1, le=100, description="JPEG quality (1-100)"
    )
    timeout: Optional[int] = Field(
        default=None, ge=1000, le=60000, description="Timeout in milliseconds"
    )
...
class ObserveRequest(BaseModel):
    ...
    include_screenshot: bool = Field(
        default=False, description="Capture a screenshot with the observation"
    )
...
class NetworkCaptureRequest(BaseModel):
    ...
    session_id: str = Field(..., description="Session ID")
    url_contains: Optional[str] = Field(
        default=None, description="Only capture responses containing this URL fragment"
    )
    resource_types: Optional[List[str]] = Field(
        default=None, description="Resource types to capture"
    )
    status_min: Optional[int] = Field(
        default=None, ge=100, le=599, description="Minimum HTTP status"
    )
    status_max: Optional[int] = Field(
        default=None, ge=100, le=599, description="Maximum HTTP status"
    )
    include_body: bool = Field(
        default=False, description="Include small text or JSON response bodies"
    )
    max_body_bytes: int = Field(
        default=65536,
        ge=1024,
        le=1048576,
        description="Maximum body bytes per response",
    )
...
class FetchRequest(BaseModel):
    ...
    body: Optional[Union[str, bytes]] = Field(
        default=None, description="Raw request body"
    )
    ...
    backend: FetchBackend = Field(
        default=FetchBackend.AUTO, description="Fetch backend"
    )
    session_id: Optional[str] = Field(
        default=None, description="Optional browser session cookie source"
    )
    impersonate: Optional[str] = Field(
        default="chrome", description="curl_cffi impersonation target"
    )
    save_to_downloads: bool = Field(
        default=False, description="Save response body into SURF downloads"
    )
    download_filename: Optional[str] = Field(
        default=None, max_length=255, description="Optional download filename"
    )
    output_dir: Optional[str] = Field(
        default=None,
        max_length=1000,
        description="Optional caller-visible output directory",
    )
    overwrite: bool = Field(
        default=False, description="Overwrite an existing file in output_dir"
    )
...
class SessionTouchRequest(BaseModel):
    ...
    reason: Optional[str] = Field(
        default=None, max_length=200, description="Optional heartbeat reason"
    )
...
class SessionReapRequest(BaseModel):
    ...
    dry_run: bool = Field(
        default=False,
        description="Report sessions that would be reaped without closing them",
    )
...
class DownloadClickRequest(BaseModel):
    ...
    session_id: str = Field(..., description="Session ID")
    selector: str = Field(
        ..., max_length=1000, description="Selector that triggers a download"
    )
    timeout: int = Field(
        default=60000, ge=1000, le=300000, description="Timeout in milliseconds"
    )
    filename: Optional[str] = Field(
        default=None, max_length=255, description="Optional saved filename"
    )
    output_dir: Optional[str] = Field(
        default=None,
        max_length=1000,
...
class BatchRequest(BaseModel):
    ...
    max_concurrent: int = Field(
        default=3, ge=1, le=10, description="Maximum concurrent operations"
    )
...
class SessionResponse(BaseResponse):
    ...
    success: bool = Field(
        default=True, description="Always true for successful session creation"
    )
...
class NetworkCaptureResponse(BaseResponse):
    ...
    success: bool = Field(
        default=True, description="Network capture operation success status"
    )
...
class SessionConfig(BaseModel):
    ...
    profile_id: str = Field(
        default="default", description="Persistent browser profile ID"
    )
    ...
    background_headed: bool = Field(
        default=True, description="Place headed browser windows off-screen by default"
    )
    persist_profile: bool = Field(
        default=True, description="Persist browser profile data"
    )
    viewport: Dict[str, int] = Field(
        default={"width": 1920, "height": 1080}, description="Viewport dimensions"
    )
    user_agent: Optional[str] = Field(default=None, description="User agent string")
    stealth: bool = Field(default=False, description="Enable legacy stealth mode")
    stealth_strategy: StealthStrategy = Field(
        default=StealthStrategy.MINIMAL, description="Browser profile strategy"
    )
    block_resources: List[str] = Field(
        default=[], description="Resource types to block"
    )
    block_mode: BlockMode = Field(
        default=BlockMode.CONSERVATIVE, description="Ad/resource blocking mode"
    )
    content_mode: ContentMode = Field(
        default=ContentMode.COMPACT, description="Default observation mode"
    )
...
class BrowserContext(BaseModel):
    ...
    expires_at: Optional[datetime] = Field(
        default=None, description="Hard expiration timestamp"
    )
    close_reason: Optional[str] = Field(
        default=None, description="Reason the session was closed or marked expired"
    )
...
class SessionStats(BaseModel):
    ...
    interactions_performed: int = Field(
        default=0, description="Number of interactions performed"
    )
    errors_encountered: int = Field(
        default=0, description="Number of errors encountered"
    )
    total_duration: float = Field(
        default=0.0, description="Total session duration in seconds"
    )
...
class SessionData(BaseModel):
    ...
    page: Optional[Any] = Field(
        default=None, exclude=True, description="Playwright page object"
    )
...
class SessionMetrics(BaseModel):
    ...
    average_page_load_time: float = Field(
        default=0.0, description="Average page load time in seconds"
    )
...
class SessionLimits(BaseModel):
    ...
    max_duration: int = Field(
        default=300, description="Maximum session duration in seconds"
    )
    ...
    max_screenshots: int = Field(
        default=50, description="Maximum number of screenshots"
    )
    max_interactions: int = Field(
        default=500, description="Maximum number of interactions"
    )
    ...
    def check_limits(self, stats: SessionStats) -> List[str]:
        ...
        if stats.total_duration > self.max_duration:
            violations.append(
                f"Session duration exceeded: {stats.total_duration}s > {self.max_duration}s"
            )

        if stats.requests_made > self.max_requests:
            violations.append(
                f"Request limit exceeded: {stats.requests_made} > {self.max_requests}"
            )

        if stats.pages_loaded > self.max_pages:
            violations.append(
                f"Page limit exceeded: {stats.pages_loaded} > {self.max_pages}"
            )

        if stats.screenshots_taken > self.max_screenshots:
            violations.append(
                f"Screenshot limit exceeded: {stats.screenshots_taken} > {self.max_screenshots}"
            )

        if stats.interactions_performed > self.max_interactions:
            violations.append(
                f"Interaction limit exceeded: {stats.interactions_performed} > {self.max_interactions}"
            )
...
class SearchRequest(BaseModel):
    """Request model for web search (Exa primary, SearXNG fallback)"""
    ...
    provider: Optional[str] = Field(
        default=None,
        description="Override provider: 'exa' or 'searxng'. Defaults to SURF_SEARCH_PROVIDER.",
    )
    fallback: Optional[bool] = Field(
        default=None,
        description="Allow fallback to the configured secondary provider on failure.",
    )
    min_relevance: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="Minimum hybrid relevance score for a result to be returned. "
        "Defaults to SURF_SEARCH_RELEVANCE_THRESHOLD.",
    )
...
class StructuredDataRequest(BaseModel):
    ...
    content_type: str = Field(
        default="general",
        description="Type of content to extract (general, forum, news, financial)",
    )
    selector: Optional[str] = Field(
        default=None, description="CSS selector for content extraction"
    )
    timeout: Optional[int] = Field(
        default=None, description="Operation timeout in milliseconds"
    )
...
class CaptchaDetectionRequest(BaseModel):
    ...
    session_id: str = Field(..., description="Session ID")
    selector: Optional[str] = Field(
        default=None, description="CSS selector for content analysis"
    )
    timeout: Optional[int] = Field(
        default=None, description="Operation timeout in milliseconds"
    )
...
class EnhancedExtractResponse(BaseModel):
    ...
    quality_metrics: Optional[Dict[str, Any]] = Field(
        default=None, description="Content quality assessment"
    )
    captcha_detected: bool = Field(
        default=False, description="Whether CAPTCHA was detected"
    )
    captcha_reason: Optional[str] = Field(
        default=None, description="Reason for CAPTCHA detection"
    )
...
class BatchOperationRequest(BaseModel):
    ...
    parallel: bool = Field(
        default=True, description="Whether to execute operations in parallel"
    )
    max_concurrent: int = Field(
        default=5, description="Maximum concurrent operations for parallel execution"
    )
    timeout: Optional[int] = Field(
        default=None, description="Global timeout for all operations"
    )
...
class BatchOperationResponse(BaseModel):
    ...
    results: List[Dict[str, Any]] = Field(
        ..., description="Results of individual operations"
    )
    total_operations: int = Field(..., description="Total number of operations")
    successful_operations: int = Field(
        ..., description="Number of successful operations"
...
class ContentQualityMetrics(BaseModel):
    ...
    has_meaningful_content: bool = Field(
        ..., description="Whether content appears meaningful"
    )
    is_captcha: bool = Field(
        default=False, description="Whether content appears to be CAPTCHA"
    )
    captcha_reason: Optional[str] = Field(
        default=None, description="Reason for CAPTCHA detection"
    )
...
class StructuredContentData(BaseModel):
    ...
    extracted_elements: Dict[str, Any] = Field(
        default_factory=dict, description="Type-specific extracted elements"
    )
